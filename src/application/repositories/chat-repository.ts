...
export abstract class ChatRepository {
  ...
  abstract getUnreadCount(roomId: string, userId?: string): Promise<number>;
  abstract markMessagesAsRead(roomId: string, userId: string): Promise<void>;
  abstract findMessagesByRoomName(roomName: string): Promise<Message[]>;
  abstract getUnreadCountByUser(roomId: string, userId: string): Promise<number>;
}
